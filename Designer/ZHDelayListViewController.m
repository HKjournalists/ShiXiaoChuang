...
@interface ZHDelayListViewController ()

@end

@implementation ZHDelayListViewController
...
//    NSString *url = [NSString stringWithFormat:@"http://oa.sitrust.cn:8001/Tositrust.asmx/GetDelay"];
    NSMutableString *url = [NSMutableString stringWithFormat:@"%@Tositrust.asmx/GetDelay", KHomeUrl];
    ...
    [manager POST:url parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (dict == nil) {
            ...
            if ([dict[@"Delay"][@"Delay"] isKindOfClass:[NSMutableDictionary class]]) {
                self.dataMArray = [[NSMutableArray alloc] initWithObjects: dict[@"Delay"][@"Delay"], nil];

            }
            else {
                self.dataMArray = dict[@"Delay"][@"Delay"];
            }
...
//    [[ImageView share] addToView:self.view imagePathName:@"标题图-基材验收" rect:RectMake2x(40, 80, 230, 40)];
    ...
    [b setTitle:@"工地延期列表" forState:UIControlStateNormal];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSArray *header = @[@"原因", @"创建日期", @"天数",  @"数量", @"照片"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    l2.text = dict [@"delayDate"][@"text"];
    l3.text = dict [@"delayDays"][@"text"];
